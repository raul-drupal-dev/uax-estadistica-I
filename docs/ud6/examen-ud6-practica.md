...
Explicación: En un contraste F bilateral, típicamente rechazamos si F > F*{α/2} (cola derecha) o F < F*{1-α/2} (cola izquierda, menos común). Un F = 0.45 (muy cercano a 0) sí sugeriría poca dispersión en el numerador, pero el valor crítico para rechazar a la izquierda es mucho menor (cerca de 0.3). **Trampa pedagógica**: Confundir "F < 1" con "rechazar"; necesitamos comparar con el valor crítico.
</quiz>
...
Explicación: En bilateral, α = 0.05 se divide en dos colas: 0.025 cada una. Los valores críticos son ±1.96. **Trampa pedagógica**: Confundir bilateral (dos colas) con unilateral (una cola). En bilateral, usamos z*{α/2}, no z*α.
</quiz>
...
Este Z = 4.6 es muy alto (z\_{0.05} = 1.96), así que rechazaríamos H₀ en un test bilateral.
</quiz>
...
- Valor crítico aprox.: F\_{0.025; 11, 14} ≈ 3.0
...
- Cola derecha: F > F\_{α/2; 29, 24}
- Cola izquierda: F < F\_{1-α/2; 29, 24} (menos común, pero posible)

**Valor crítico típico**:

- F\_{0.025; 29, 24} ≈ 2.0 (tabla o software)
- F\_{0.975; 29, 24} ≈ 0.4–0.5
...
- Pero tipicamente, como F\_{1-α/2} es cercano a 0.3–0.4 y F = 0.45 > ese umbral, **NO rechazamos**
...
- En bilateral: z*{α/2} = z*{0.025} = 1.96
- En unilateral derecha: z*{α} = z*{0.05} = 1.645
- En unilateral izquierda: z*{α} = z*{0.05} = -1.645
...
- Valor crítico bilateral: z\_{0.025} = 1.96
...
- Tipo: **Bilateral**
- α = 0.05
- Valor crítico: t\_{19, 0.025} = 2.093
